...
| `checkpoint-mecanico.sh` | PreCompact 🔔 Automático | **El 80% del checkpoint a CERO tokens de modelo (M2/X2, auditoría 2026-09-11):** en el MISMO evento que `exportar-sesion-master` (mismo `transcript_path`), corre DETACHED (nohup + lock por-sid) el extractor `bin/checkpoint-mecanico.js` — lee el transcript en streaming (memoria acotada, medido ~130-190 MB de pico sobre un transcript de 756 MB) y saca, sin criterio: archivos tocados (Write/Edit/NotebookEdit), skills invocadas, mensajes de `git commit`, ramas/cwds vistos y los últimos mensajes del usuario VERBATIM. Escribe (atómico, tmp+rename) `.claude/memory/hilo-mental-actual.andamio.md` — un SIDECAR que el skill `checkpoint` fusiona; nunca pisa `hilo-mental-actual.md` (eso lo sigue escribiendo el modelo, con juicio). Antes de este hook, `PreCompact` guardaba los BYTES del transcript (gzip) y tiraba el SIGNIFICADO. |
